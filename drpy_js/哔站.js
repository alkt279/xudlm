...
var rule = {
    ...
    filter:{"全部":[{"key":"tid","name":"分类","value":[{"n":"番剧","v":"1"},{"n":"国创","v":"4"},{"n":"电影","v":"2"},{"n":"电视剧","v":"5"},{"n":"记录片","v":"3"},{"n":"综艺","v":"7"}]},{"key":"order","name":"排序","value":[{"n":"播放数量","v":"2"},{"n":"更新时间","v":"0"},{"n":"最高评分","v":"4"},{"n":"弹幕数量","v":"1"},{"n":"追看人数","v":"3"},{"n":"开播时间","v":"5"},{"n":"上映时间","v":"6"}]},{"key":"season_status","name":"付费","value":[{"n":"全部","v":"-1"},{"n":"免费","v":"1"},{"n":"付费","v":"2,6"},{"n":"大会员","v":"4,6"}]}],"时间表":[{"key":"tid","name":"分类","value":[{"n":"番剧","v":"1"},{"n":"国创","v":"4"}]}]},
    play_parse:true,
    ...
    二级:'js:' +
        'function zh(num){' +
            'let p="";' +
            'if(Number(num)>1e8){p=(num/1e8).toFixed(2)+"亿"}' +
            'else if(Number(num)>1e4){p=(num/1e4).toFixed(2)+"万"}' +
            'else{p=num}' +
            'return p' +
        '}' +
        ...
        // 自定义简介内容 - 方便编辑
        'let customContent="阿当" + (dec || "暂无简介");' +
        'let vod={vod_id:id,vod_name:title,vod_pic:pic,type_name:typeName,vod_year:date,vod_area:areas,vod_remarks:remark,vod_actor:status,vod_director:score,vod_content:customContent};' +
        'let ja=jo["episodes"].filter(ep=>!ep.title.includes("预告") && !(ep.badge && ep.badge.includes("预告")));' +
        'let playurls1=[];' +
        'let playurls2=[];' +
        'ja.forEach(function(tmpJo){' +
            ...
            'let cid=tmpJo["cid"];' +
            'let link=tmpJo["link"];' +
            'let part="第" + tmpJo["title"] + "话";' +
            'if(tmpJo["long_title"] && tmpJo["long_title"].trim()) {' +
                'part += " " + tmpJo["long_title"];' +
            '}' +
            'let badges = [];' +
            'if(tmpJo["badge"] && tmpJo["badge"].trim()) {' +
                'badges.push(tmpJo["badge"]);' +
            '} else if(tmpJo["status"] === 13) {' +
                'badges.push("需要会员");' +
            '}' +
            'if(badges.length > 0) {' +
                'part += "[" + badges.join("|") + "]";' +
            ...
            // 在直接链接后添加弹幕标识，格式：link|eid_cid
            'playurls2.push(part+"$"+link+"|"+eid+"_"+cid)' +
        '});' +
        'let playUrl=playurls2.length>0?playurls2.join("#"):"";' +
        // 播放源名称 - 方便编辑
    ...
    lazy:`js:
        // 检查是否是带弹幕信息的直接链接格式：link|eid_cid
        if (input.includes("|")) {
            let parts = input.split("|");
            let directLink = parts[0];
            let danmakuInfo = parts[1];
            let ids = danmakuInfo.split("_");
            ...
            // 处理普通直接链接，尝试提取ep_id获取弹幕
            let dan = "";
            let epMatch = input.match(/ep(\d+)/);
            if (epMatch) {
                let ep_id = epMatch[1];
                // 通过ep_id获取cid来获取弹幕
                let apiUrl = "https://api.bilibili.com/pgc/view/web/season?ep_id=" + ep_id;
                try {
                    let apiHtml = request(apiUrl);
                    let apiRoot = JSON.parse(apiHtml);
                    if (apiRoot.code === 0) {
                        let episodes = apiRoot.result.episodes;
                        let currentEp = episodes.find(ep => ep.id.toString() === ep_id);
                        if (currentEp) {
                            dan = 'https://api.bilibili.com/x/v1/dm/list.so?oid=' + currentEp.cid;
                        }
                    }
                } catch (e) {
                    // 弹幕获取失败，静默处理
